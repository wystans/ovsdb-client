...
function Client(port, host, certs) {
    ...
    this._updateEmitter = new events();
...
Client.prototype.connect = function (callback) {
    ...
    this._connection.expose('update', function (_uuid, result) {
        ...
        currentUpdate = self._updates[0];

        self._updateEmitter.emit('change', currentUpdate);


        // Get the table that was modified
        for (var key in currentUpdate) {

            var table = key;
            for (var updateType in currentUpdate[table]) {


                if (currentUpdate[table].hasOwnProperty(updateType)) {

                    var current = currentUpdate[table][updateType];

                    if (current.hasOwnProperty('new') && current.hasOwnProperty('old')) {
                        self._updateEmitter.emit('update', currentUpdate);
                    }
                    else if (current.hasOwnProperty('new')) {
                        self._updateEmitter.emit('insert', currentUpdate);
                    }
                    else if (current.hasOwnProperty('old')) {
                        self._updateEmitter.emit('delete', currentUpdate);
                    }
                    else {
                        console.log(`Error determining update type (update, insert, or delete)`);
                    }
                }
            }
        }
...
Client.prototype.start_monitor = async function (table, columns, callback) {
    var self = this;
    var dbname = "Open_vSwitch";
    var remote = this._connection._remote;
